...
def add_address(session, city, country) -> uuid.UUID:
    existing_address = (
        session.query(Address).filter_by(city=city, country=country).first()
    )
...
def create_work_experience_instances(
    resume_id: uuid.UUID, data: list[dict]
) -> list[WorkExperience]:
    work_experiences = []
    try:
        for work_exp in data:
            ...
            start_date, end_date = get_start_end_dates(
                work_exp["start_date"], work_exp["end_date"]
            )
    ...
    for name, value in data.items():
        if len(value) > 0:
            try:
                ...
                print(
                    f"Warning: '{name}' is not a valid social media platform. Skipping..."
                )
                continue

            social_media_instance = SocialMedia(
                name=platform, link=value, resume_id=resume_id
            )
...
def create_language_instances(resume_id: str, data: list[dict]) -> list[Language]:
    ...
    for item in data:
        ...
        language_instance = Language(
            name=name, fluent_level=fluent_level, resume_id=resume_id
        )
...
def get_sample_resume(template: Template):
    ...
    """,
        ...
        address={"country": "Myanmar", "city": "Taunggyi"},
        template=template,
    )
    social_media_info = {
    ...
    eduI_info = [
        {
            ...
            "school": "Info Myanmar University",
            "location": "Yangon",
    ...
    sample_resume.work_experiences = create_work_experience_instances(
        sample_resume.id, work_exp_info
    )
    ...
    sample_resume.social_media = create_social_media_instances(
        sample_resume.id, social_media_info
    )
...
def _save_file(
    file, upload_folder_path: str, custom_extension: Optional[str] = None
) -> Tuple[str, str]:
    file_id = uuid.uuid4()
